...
client.channels.get(config.old_log_channel).fetchMessages()
    .then(messages => {
        ...
        messages.map(msg => {
            ...
            let id = log[0].split("Username: ")[1].split("#")[1].replace("(", "").replace(")", "").substring(5)
            console.log(id)
            ...
            for (i = 0; i < log.length; i++) { 
                if (log[i].includes("Evidence")) {
                    log[i].replace("Evidence: ", "")
            ...
            // explaination
            let details = log[8].substring(13)
            console.log(details)
            ...
            //await Moderation.addLogLegacy(datetime, msg['id'], username, id, staff, staffId, details, action)
        })
    })
    .catch(console.error)
})
