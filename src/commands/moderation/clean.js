...
module.exports = class CleanCommand extends Command {
    constructor(client) {
        ...
        __you:__ Messages sent by Auto Mod
    ...
    async run(msg, { filter, limit, member }) {
        ...
        if (filter) {
            ...
            var messages = await msg.channel.fetchMessages({ limit: limit }).catch(err => null)
            var messagesToDelete = messages.filter(messageFilter)
            msg.channel.bulkDelete(messagesToDelete.array().reverse()).catch(err => null)

            return null
        }

        var messagesToDelete = await msg.channel.fetchMessages({ limit: limit }).catch(err => null)
        msg.channel.bulkDelete(messagesToDelete.array().reverse()).catch(err => null)
